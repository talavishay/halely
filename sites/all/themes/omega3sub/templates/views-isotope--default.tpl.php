...
?>
  <div id="isotope-container">
    ...
    <?php foreach ( $rows as $id => $row ): ?>
      ...
      $classlistSep = explode("-", $classlist);
	  
      ?>
      <div class="isotope-element isotope-element-<?php print $count; ?> <?php
       foreach($classlistSep as $k => $v){
   			$num = intval($v);
       		if($num !== 0){
       			print " nid-".$v." ";		
       		} else {
       ...
       ?>" data-category="<?php print $classlist; ?>" <?php print $bgstyle ?>>
        <?php print $row; ?>
      </div>
      <?php 
      // reset
